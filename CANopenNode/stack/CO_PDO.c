        ...
        uint8_t                *pIsMultibyteVar)
{
    ...
    *pLength += dataLen;
	...
	uint8_t maxssub = SDO->OD[entryNo].maxSubIndex;
...
static uint32_t CO_TPDOconfigMap(CO_TPDO_t* TPDO, uint8_t noOfMappedObjects){
    ...
    for(i=noOfMappedObjects; i>0; i--){
		...
		uint8_t testfassr = TPDO->em->errorStatusBits[3];
        /* write PDO data pointers */
#ifdef CO_BIG_ENDIAN
        if(MBvar){
            for(j=length-1; j>=prevLength; j--)
        ...
        uint16_t                CANdevRxIdx)
{
	...
	uint8_t tester=RPDO->em->errorStatusBits[3];
    return CO_ERROR_NO;
}
        ...
        uint16_t                CANdevTxIdx)
{
	...
	uint8_t testige = TPDO->em->errorStatusBits[3];
...
{
	volatile bool valid = TPDO->valid;
	volatile uint8_t opstate = *TPDO->operatingState;
	volatile uint8_t good_opstate = CO_NMT_OPERATIONAL;
