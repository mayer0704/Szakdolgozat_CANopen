        ...
        uint16_t                bitRate)
{
	...
	uint8_t test_err= CO->em->errorStatusBits[3];
    for (i=0; i<CO_NO_SDO_SERVER; i++)
    {
...
test_err= CO->em->errorStatusBits[3];
        err = CO_SDO_init(
                CO->SDO[i],
...
test_err= CO->em->errorStatusBits[3];
    for(i=0; i<CO_NO_RPDO; i++){
        ...
        err = CO_RPDO_init(//itt van az error
                CO->RPDO[i],
                CO->em,
                CO->SDO[0],
                CO->SYNC,
        ...
        uint16_t               *timerNext_ms)
{
	...
	uint8_t testkldfjkkl=CO->em->errorStatusBits[3];
    for(i=0; i<CO_NO_SDO_SERVER; i++){
        CO_SDO_process(
                CO->SDO[i],
	...
	testkldfjkkl=CO->em->errorStatusBits[3];
    CO_EM_process(
            CO->emPr,
            NMTisPreOrOperational,
...
void  CO_CAN1InterruptHandler(uint8_t handle, uint8_t event)
{
    CO_CANinterrupt(CO->CANmodule[0], handle, event);//CO->CANmodule paramétert itt megadhatjuk
